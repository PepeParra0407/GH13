#Análisis para Hb216 que es la alfa-amilasa de Thermotoga maritima
...
def msa_to_df(fasta):
    ...
    if not 'Hb216.tm' in seqs:
        sys.exit(f"Can't find seq for Hb216.tm") # Adjusted error message for clarity
    Hb216= seqs['Hb216.tm']
    chars = {name:list(seq) for name,seq in seqs.items()}
    return pd.DataFrame(chars).T, Hb216
...
def Hb216_pos(pos):
    Hb216p = pos +1 - Hb216[0:pos].count('-')
    Hb216aa = Hb216[pos]
    return Hb216p,Hb216aa
...
if __name__ == '__main__':
    ...
    fasta = "/content/jose_seqs.linsi.fasta" # Directly assign the fasta file path
    # fasta = sys.argv[1]
    fasta3di = fasta.replace('_aa','_3di')
    trim= fasta.replace('.fa','_trim.fa')
    df,Hb216 = msa_to_df(fasta)
    print(df.shape)
    ...
    print(''.join(Hdf.loc['Hb216.tm'].astype(str))) # This line is correct, 'Hb216.tm' is in Hdf
    ...
    for k,v in p_values.items():
        ...
        Hb216p,Hb216aa = Hb216_pos(k)
        print(f"  {k}\t{renum[k]}\t{Hb216aa}\t{Hb216p}\t{v:.3g}\t{compare}\taa")
    ...
    for class_name, categos in classes.items():
        ...
        for k,v in p_values.items():
            Hb216p, Hb216aa = Hb216_pos(k) # Unpack the tuple here
            compare = compare_symbols(H_pcnt[k],T_pcnt[k])
            print(f"  {k}\t{renum[k]}\t{Hb216aa}\t{Hb216p}\t{v:.3g}\t{compare}\t{class_name}\t{categos}")
    ...
    # print(''.join(Hdf.loc['Hb216'].astype(str)))
    ...
    for k,v in p_values.items():
        ...
        compare = compare_symbols(H_pcnt[k],T_pcnt[k])
        Hb216p, Hb216aa = Hb216_pos(k) # Unpack the tuple here
        print(f"  {k}\t{renum[k]}\t{Hb216aa}\t{Hb216p}\t{v:.3g}\t{compare}\t3di")
